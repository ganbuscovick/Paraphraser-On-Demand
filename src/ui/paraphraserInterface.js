...
function ensureButtonExists() {
    ...
    let refButton = document.getElementById('qr_toggle_button');
    if (!refButton) {
        // fallback
        refButton = document.querySelector('#extensions_button, .extensionsMenuBtn, .extensions_button');
    }

    if (!refButton) {
        console.warn("Paraphraser: não achei refButton (qr_toggle_button nem extensions_button)");
        return;
    }

    if (!paraphraseButton) {
        paraphraseButton = createButtonElement();
        refButton.insertAdjacentElement('afterend', paraphraseButton);
        attachListener(paraphraseButton);
        console.log("Paraphraser: botão criado após", refButton.id || refButton.className);
...
function attachListener(buttonElement) {
    ...
    buttonElement.addEventListener('click', async () => {
        try {
            ...
            // seleciona última msg de usuário
